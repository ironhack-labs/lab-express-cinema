...
Movie.deleteMany()
    .then(()=> 'Movie database deleted')
    .catch((error) =>  console.error('Error while deleting the Movie database:', error))
    .then(()=> { 
        Movie.insertMany(movies)
        .then(mov => mov.forEach(m => console.log(m.title)))
        .catch(error => console.error('Autopopulate movies error:', error))
    }) 
