...
// fonction qui permet de seeder ta base de données
// FICHIER SEED INDEPENDANT DE NOTRE APP = QUAND ON LANCE FICHIER SEED, ON LANCE PAS APP
// DONC ON DOIT LANCER LE SEED (node bin seed.js)
async function insertTestMovies() {
    try {
      await MovieModel.deleteMany(); // au cas où, je supprime tout = on remet la db à zéro
      const insertedMovies = await MovieModel.insertMany(movies); // insert docs in db
         console.log("test movies seed done !");
