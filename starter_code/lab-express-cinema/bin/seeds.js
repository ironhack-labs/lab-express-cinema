...
async function fillInDatabase(data) {
    try {
        let response = await Movie.insertMany(data);

        if(response.length) {
            console.log("Data was sent in database!");
        }
    } catch(err) {
        console.log("There is an error with your data!", err);

    }

}

fillInDatabase(movies);