...
mongoose
    .connect('mongodb://localhost/9-express-cinema', {useMongoClient: true})
    .then(() => {
        console.log( 'Connected to Mongo!' );
    })
    .catch(( err ) => {
        console.log( "Errr, didn't werk...", err );
    });
...
Movie.create( movies )
    .then(() => {
        console.log( `Yay, just created ${movies.length} movies!` );
    })
    .catch( err => {
        console.log( "Wew no wey!", err );
    });