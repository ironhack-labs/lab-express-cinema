...
const MONGO_URI =
  process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/lab-express-movies";
...
async function seed() {
    try {
        ...
        let moviesIntoDb = await Movie.create(movies);
        console.log(`Created ${moviesIntoDb.length} movies in the database`);
    } catch (error) {
        console.log(error);
    }
}
