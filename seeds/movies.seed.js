...
const Mongo_URI = process.env.Mongo_URI || 'mongodb://localhost/lab-express-cinema'; //(lab-express-cinema es el nombre que se le data a la nueva base de datos que se cree.)
...
Movie.create(movies)
.then(moviesFromDB =>{
    console.log(`Creacion ${moviesFromDB.length} movies`)
    mongoose.connection.clase()
  })
.catch(error => console.log('Un error ocurrio: ', error)
  );