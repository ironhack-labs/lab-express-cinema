...
//Un seed es un CREATE , pero desde aqui
const movies = [
    {
  ...
  // Importamos nuestro modelo, por convencion es el nombre de la coleccion en SINGULAR
  // "Traemos" la pagina del SCHEMA
  const Movie = require('../models/Movie.model')

 //3000
 //27017 en este puerto esta conectado MongoDB

 
 // mongoose.connect(ruta)→ nos devuelve una promesa, por convencion se pone en lineas distintas
mongoose
.connect("mongodb://127.0.0.1:27017/lab-express-cinema") //crea la base de datos
.then(async()=>{ //un .then debe recibir un callback
    console.log('se ha conectado a la seed')
    await Movie.deleteMany()
    await Movie.insertMany(movies) //metodo para crear el modelo"
})
.then(()=>{
    mongoose.disconnect()
...
// LA BASE DE DATOS EN UN SEED HAY QUE CERRARLA
