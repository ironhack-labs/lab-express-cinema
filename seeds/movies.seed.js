...
const MONGO_URI = process.env.MONGODB_URI || 'mongodb://localhost/lab-express-cinema'; // declare a variable if there is an env with MONGO-Uri we will use it,otherwise mongodb...

mongoose.connect(MONGO_URI, { //try to connect database with some settings
...
Movie.create(movies)
  .then(moviesFromDB => {
    console.log(`Movies ${moviesFromDB.length} movies`);
    mongoose.connection.close();
  })
  .catch(err => console.log(`An error occurred while creating new movie from the DB: ${err}`));