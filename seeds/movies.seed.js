...
mongoose
  .connect(MONGO_URI)
  .then((dataBase) => {
    console.debug(`Connected to the database: "${dataBase.connection.name}"`);
    return Movie.create(movies)
  })
  .then((moviesCreated) => {
    console.debug(`We create ${moviesCreated.length} movies, the first is ${moviesCreated[0].title}`);
  })
  .catch((error) => console.error('An error occurred while creating movies from the Data Base'))
