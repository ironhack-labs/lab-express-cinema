...
const connectionString = "mongodb://localhost:27017/lab-express-cinema"

mongoose
    .set('strictQuery', false)
    .connect(connectionString)
    .then(() => {
        return Movie.collection.drop()                               // vaciar colección
    })
    .then(() => {
        return Movie.insertMany(movies)                               // transacción
    })
    .then(movies => {
        console.log('Se han creado', movies.length, 'libros')
    })
    .finally(() => {
        mongoose.connection.close()                                 // cierre
    })
